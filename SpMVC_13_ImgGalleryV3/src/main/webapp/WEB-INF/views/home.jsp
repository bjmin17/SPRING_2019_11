		...
		*/
		...
		var img_context = function(key,option){
			...
			if(key == "delete"){
				...
				// img tag 를 찾아서
				// 그 태그의 attribute 중에 src 값을 가져와라 : "이미지 이름"
				...
				// 제일 끝에 있는 값만 가져오기,,, 파일 이름만 추출
				let img_file = img_srcs[img_srcs_len - 1]
				
				// 파일 이름을 서버로 보내기
				// Post와 Get 방식 중 POST로 보내기(PUT, DELETE, HEAD 등이 더 있음)
				$.post(
						// 보낼 주소
						"${rootPath}/rest/main_image",
						// img_pcode에 img_pcode를 담아 보낼것
		...
		$.contextMenu({
			...
			items : {
				...
				'delete' : {name:'삭제',icon:'delete'},
				'main' : {name:'대표이미지 설정',icon:'far fa-check-circle'}
			}
