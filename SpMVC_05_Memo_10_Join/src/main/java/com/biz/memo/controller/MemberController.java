 ...
 * 1. Model(ModelAndView).addAttribute()
 * 2. SessionAttributes() ModelAndAttribute() 설정 후 Model에 담기
 * 3. HttpSession.setAttribute()에 담기
 * 
 * Model : 일회용데이터 
 * 		Controller >> view로 보내기만을 위한 데이터
 * 		Model에 담긴 데이터를 다시 server로 보내려면
 * 		input tag에 값을 담아서 다시 POST로 전송
 * 
 * SessionAttributes + Model : 일회용데이터이면서 Session에 유지되는 데이터
 * 		view(input view)에서Spring form tag에 값을 담으면
 * 		이후에 자유롭게 서버로 전송을 할 수 있다.
 * 		input, update 코딩을 간편하게 사용하는 용도
 * 		spring에서는 SessionAttribute일 경우 Garbage Collection이 적용된다.
 * 
 * HttpSession : 주로 login과 관련된 데이터를 유지하기 위한 용도
 * 		특별히 시간설정을 하거나, 값을 remove 하거나
 * 		서버가 멈출 때까지 그 값을 유지하고
 * 		모든 controller, 모든 view에서
 * 		값을 참조할 수 있다.
 * 		서버의 메모리 공간을 많이 차지하여 
 * 		서버 성능에 문제를 일으키기도 한다. 
...
public class MemberController {
	...
	public String login(UserDTO userDTO, Model model, HttpSession httpSession) {
		...
		if(userDTO.getU_id().equalsIgnoreCase(getUserDTO.getU_id()) && userDTO.getU_password().equals(getUserDTO.getU_password())) {
