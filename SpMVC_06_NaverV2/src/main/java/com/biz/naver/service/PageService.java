...
public class PageService {
	...
	public PageDTO makePagination(long totalCount, long currentPageNo) {
		...
		// naver는 페이지를 검색할 때 1000페이지가 넘어가면 오류를 보낸다.
...
//		if(finalPageNo > 1000) finalPageNo = 1000; // if 문을 이용한 연산
		...
		// 첫 페이지인가, 마지막 페이지인가를 검사해서 flag를 세팅
		// bYes = 3 == 3;
		boolean isNowFirst = currentPageNo == 1;
		boolean isNowFinal = currentPageNo == finalPageNo;
		
		// 하단에 리스트로 보여줄 페이지 계산
		// cur : 3 이면 1 ~ 5, cur : 10이면 8 ~ 12까지 이런식으로 보여주기 위한 계산
		long startPageNo = ((currentPageNo - 1) / pageCount) * this.pageCount + 1;
