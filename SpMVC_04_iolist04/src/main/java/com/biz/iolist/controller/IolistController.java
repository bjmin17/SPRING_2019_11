...
public class IolistController {
	...
	public String ioHome(Model model) {
		 ...
		 * 이 때, 이름 "IOLIST"를 대문자로 사용하는 이유는
		 * 대소문자가 섞이면 문자 차이로 인한 오류가 발생하거나
		 * 값을 표시하지 못하는 일이 있는데,
		 * 이를 방지하기 위함이다. 
		 */
	...
	public String list(Model model) {
		...
		/*
		 * 새로 입력을 수행할 때, Default 값을 만들어서 form에 세팅하기
		 */
		// 서버의 현재 날짜를 문자열로 생성
		Date date = new Date();
	 ...
	 * 발생하는 오류이다.
	 */
	// 입력 form에서 데이터를 입력하고 전송(저장)버튼을 클릭했을 때
	// 데이터를 수신할 method
	 ...
	 * 일단 seq 칼럼으로 조회를 하기 위한 쿼리값인 id는
	 * 문자열로 받는다.
