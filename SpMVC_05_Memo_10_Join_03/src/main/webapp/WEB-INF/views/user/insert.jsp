...
<script>
	...
	// let idcheck = function() {  이라고 함수를 만들 수도 있지만, 브라우저가 지원 안 할 수도
	// 표준함수 선언 방식
	function idcheck() {
		let top = 500
		...
		openWin = window.open("${rootPath}/user/idcheck?u_id=" + u_id,"_blank", status)
...
<fieldset>
	...
	<form:form modelAttribute="userDTO" autocomplete="on" class="memo-form">
		...
		<div class="in-box-border">
			<form:input path="u_id" type="text" class="in-box" placeholder="사용자 ID를 입력하고 Enter..."/>
		...
		<div class="in-box-border">
			<form:input type="password" path="re_password" placeholder="비밀번호를 다시 한번 입력하세요"/>
			<form:errors path="re_password" class="in-error"/>
		</div>
		
		<div class="in-box-border">
			<form:input path="u_name" class="in-box" placeholder="이름를 입력하세요"/><br/>
