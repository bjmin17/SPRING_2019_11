...
public class DeptController {
	 ...
	 * /dept/list path로 request 했을 때,
	 * list() method가 호출된다.
	 */
	// class의 /dept와 method /list를 묶어서 /dept/list path로 request 했을 때
	// list() method가 호출된다.
	...
	public ModelAndView list() {
		 ...
		 * Dispatcher에게 객체를 리턴해주는 형식으로 사용
		 * 
		 * Model은 객체만 담고
		...
		// return String이 아닌 ModelAndView를 사용
		return mView;
	}
	...
	public String delete(String d_code, Model model) {
		int ret = dService.delete(d_code);
		
		// return null; 하면 난리남, 오버플로우
		return "redirect:/dept/list";
	}
	...
	public String update(DeptDTO deptDTO, Model model) {
		...
		// return return "redirect:/dept/view?d_code=?" + deptDTO.getD_code???????????;
		return "redirect:/dept/list";
	}
}
